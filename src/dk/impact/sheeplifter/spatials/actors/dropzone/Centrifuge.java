...
public class Centrifuge extends Node{
    ...
    private void buildTrail(int trailSectionCount){
        ...
        Texture t1 = TextureManager.loadTexture(
                Centrifuge.class.getClassLoader().getResource("res/geometry/trail.png"),
    ...
    public void update(float timeDelta){
        rotDir.set(FastMath.sin(rotDirAngle * angleSpeedX),
                   FastMath.cos(rotDirAngle * angleSpeedY),
                   FastMath.cos(rotDirAngle * angleSpeedZ));
        ...
        if(state == States.accelerating){
            ...
            // Shoot out a spatial into space?
            if(speed >= 22 && trailingSpatials.size() > 0 &&
               timeSinceLastShootOut > shootOutdelay &&
               (shootOutStarted || isLeadPosInVertAngle())){
        ...
        // Update spatials being shot out.
